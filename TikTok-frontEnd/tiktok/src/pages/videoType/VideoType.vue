<template>
  <v-container class="px-4 py-6">
    ...
    <!-- Empty State -->
    <v-card
      v-if="!loadingMore && typeVideos.length === 0 && !initialLoading"
...
async function loadMore() {
  ...
  try {
    ...
    const response = await getVideosByTypeId(
      props.id,
      page.value,
      pageSize.value
    );
    ...
    if (page.value === 1) {
      typeVideos.value = processedVideos;
    } else {
      typeVideos.value = [...typeVideos.value, ...processedVideos];
    }

    // Check if there are more videos to load
    if (
      processedVideos.length < pageSize.value ||
      typeVideos.value.length >= totalCount
    ) {
      hasMore.value = false;
    } else {
      page.value++;
    ...
    console.log("当前已加载:", typeVideos.value.length);
    console.log("总数:", totalCount);

    console.log(
      `Loaded ${processedVideos.length} videos, total: ${typeVideos.value.length}`
    );
    console.log(typeVideos.value);
  } catch (error) {
