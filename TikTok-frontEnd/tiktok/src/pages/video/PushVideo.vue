<template>
  <v-container class="px-4 py-6">
    ...
    <!-- Empty State -->
    <v-card
      v-if="!loadingMore && pushedVideos.length === 0 && !initialLoading"
    ...
    <div v-if="!hasMore && pushedVideos.length > 0" class="text-center py-8">
      <v-divider class="mb-4" />
      <p class="text-body-2 text-medium-emphasis">
        已加载全部内容 ({{ pushedVideos.length }} 个视频)
      </p>
    </div>
...
async function loadMore() {
  ...
  try {
    ...
    if (page.value === 1) {
      pushedVideos.value = processedVideos;
    } else {
      pushedVideos.value = [...pushedVideos.value, ...processedVideos];
    }

    // Check if there are more videos to load
    if (
      processedVideos.length < pageSize.value ||
      pushedVideos.value.length >= totalCount
    ) {
      hasMore.value = false;
    } else {
      page.value++;
    ...
    console.log("当前已加载:", pushedVideos.value.length);
    console.log("总数:", totalCount);

    console.log(
      `Loaded ${processedVideos.length} videos, total: ${pushedVideos.value.length}`
    );
    console.log(pushedVideos.value);
  } catch (error) {
