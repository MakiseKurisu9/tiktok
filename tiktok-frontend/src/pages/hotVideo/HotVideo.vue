<template>
  <v-container class="px-4 py-6">
    ...
    <v-row v-else class="video-grid">
      <v-col v-for="video in hotVideos" :key="video.id" cols="12" sm="6" md="4" lg="3" class="video-col">
    ...
    <v-card v-if="!loadingMore && hotVideos.length === 0 && !initialLoading" class="text-center py-16 my-8" variant="outlined">
    ...
    <div v-if="!hasMore && hotVideos.length > 0" class="text-center py-8">
      ...
      <p class="text-body-2 text-medium-emphasis">已加载全部内容 ({{ hotVideos.length }} 个视频)</p>
    </div>
...
function onLiked(videoId, isLiked) {
  const v = hotVideos.value.find((x) => x.id === videoId);
...
async function loadMore() {
  ...
  try {
    ...
    if (page.value === 1) hotVideos.value = processed;
    else hotVideos.value.push(...processed);

    if (processed.length < pageSize.value || hotVideos.value.length >= totalCount) hasMore.value = false;
    else page.value++;
  } catch (error) {
...
async function refreshVideos() {
  page.value = 1; hasMore.value = true; hotVideos.value = []; initialLoading.value = true;
  await loadMore();
}
